...
class TrajLocPredExecutor(AbstractExecutor):
    ...
    def train(self, train_dataloader, eval_dataloader,accelerate_flag=False,accelerator=None):
        ...
        for epoch in range(self.config['max_epoch']):
            ...
            #####Training stage ###############################
            self.model, avg_loss = self.run(train_dataloader, 
                                            self.model,
    ...
    def load_model(self, cache_name):
        ...
        for name, param in model_state.items():
            if name in current_model_state:
                if param.size() != current_model_state[name].size():
                    if param.size(0) > current_model_state[name].size(0):
                        current_model_state[name] = param[:current_model_state[name].size(0), ...]
                     # 如果加载的参数维度大于当前模型，只使用前部分
                    else:
                        # 如果当前模型层比载入模型层大，使用载入的权重填充对应部分，其余部分用随机值填充
                        new_param = current_model_state[name]
                        new_param[:param.size(0), ...] = param
                        if param.size(0) < new_param.size(0):
                            shape_difference = new_param.size(0) - param.size(0)
                            new_param[param.size(0):, ...] = torch.rand(shape_difference, *param.size()[1:])
                        current_model_state[name] = new_param
                else:
                    current_model_state[name] = param
            else:
                raise KeyError(f"Missing {name} in the current model parameters.")
    ...
    def evaluate(self, test_dataloader):
        ...
        for batch in test_dataloader:
            #print('loc',batch['current_loc'])
            batch.to_tensor(device=self.config['device'])
            scores = self.model.predict(batch)
            val, index = torch.topk(scores, 10, 1)
            #print('idx',index)
            #print('tgt',batch['target'])
            #print('*'*10)
    ...
    def run(self, data_loader, model, lr, clip,accelerate_flag=None):
        model.train(True)
