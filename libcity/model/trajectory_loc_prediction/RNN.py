...
class RNN(AbstractModel):
    ...
    def forward(self, batch):
        his_loc=batch['history_loc']
        #print(his_loc.shape)
        cur_loc = batch['current_loc']
        #print(cur_loc.shape)
        loc=torch.concat([his_loc,cur_loc],dim=1)
        #print('loc',loc)
        his_tim=batch['history_tim']
        #print(his_tim.shape)
        cur_tim = batch['current_tim']
        # print(cur_tim.shape)
        tim=torch.concat([his_tim,cur_tim],dim=1)
        #print('loc',loc,loc.max())
        #print('tim',tim,tim.max())
        loc_len = [his_loc.size(1) + cur_loc.size(1)] * loc.size(0)
        ...
        val, index = torch.topk(score, 10, 1)
        #print('idx',index)
        #print('tgt',batch['target'])
        hit=0
        for i in range(len(batch['target'])):
            if batch['target'][i] in index[i]:
                hit+=1
        #print(hit,len(batch['target']),hit/len(batch['target']))
        #print('*'*10)
        return score
...
class RNN(AbstractModel):
    ...
    def __init__(self, config, data_feature):
        super(RNN, self).__init__(config, data_feature)
        self.loc_size = 2
        self.loc_emb_size = config['dim_feature']
        self.tim_size = data_feature['day_size']
        self.tim_emb_size = config['dim_feature']
        self.hidden_size = config['dim_feature']
        self.device = config['device']
        self.rnn_type = config['rnn_type']
        self.evaluate_method = config['evaluate_method']
